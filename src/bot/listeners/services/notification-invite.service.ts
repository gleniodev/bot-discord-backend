...
export class NotificationService {
  ...
  public async sendWelcomeEmbed(data: WelcomeData) {
    const { member, inviteUrl, invitedByTag, inviteCreatorTag, inviteUses } =
      data;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          ...
          value: invitedByTag
            ? `Convidado por **${invitedByTag}**`
            : 'Convite direto / indefinido',
        },
        {
      ...
      .setFooter({
        text: `Made with ♥ | ID: ${member.user.id} • ${new Date().toLocaleString('pt-BR')}`,
      });
  ...
  public async sendGoodbyeEmbed(member: GuildMember) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: '⚠️ Motivo da saída:',
          value: 'Saiu do servidor',
          inline: false,
        },
      )
      .setFooter({
        text: `Atlanta - Cavalaria • ${new Date().toLocaleString('pt-BR')}`,
      });
